# 说明：以下 -dontwarn 均基于 R8 生成的 missing_rules.txt（2026-09-02 Release 采集）收窄为精确类清单。
# 触发来源：POI/xslf 与 xmlbeans 的可选依赖（java.awt、javax.imageio、pdfbox、batik、saxon、
# javaparser、bouncycastle、ant、maven、osgi、xml-security 等），Android 运行时不存在且不调用，
# 仅被 POI 可选功能引用，R8 全量分析时告警，故按类精确抑制。POI 本体由下方 keep 完整保留，
# 运行正确性由 Release 真机解析样表验证兜底。
...
# ===== 精确 -dontwarn（来自 missing_rules.txt）=====
